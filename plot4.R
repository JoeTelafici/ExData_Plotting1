...
createplot4 <-function ()
{
  ...
  require(gridExtra)
  plot1 <- ggplot(powerdata, aes(Date, Global_active_power)) + geom_line() + scale_x_datetime(date_labels = "%a", date_breaks ="1 day") + xlab("") + ylab("Global Active Power") + theme( panel.background = element_rect(fill = NA, colour = "black"))
  plot2 <- ggplot(powerdata, aes(Date, Voltage)) + geom_line() + scale_x_datetime(date_labels = "%a", date_breaks ="1 day") + xlab("datetime") + ylab("Voltage") + theme( panel.background = element_rect(fill = NA, colour = "black")) + scale_y_continuous(breaks = c(234, 238, 242, 246))
  plot3 <- ggplot(powerdata, aes(x=Date)) + geom_line(aes(y=Sub_metering_1, colour="Sub_Metering_1")) + geom_line(aes(y=Sub_metering_2, colour="Sub_Metering_2")) + geom_line(aes(y=Sub_metering_3, colour="Sub_Metering_3")) + scale_x_datetime(date_labels = "%a", date_breaks ="1 day") + xlab("Date") + ylab("Energy sub metering") + scale_colour_manual(values=c("black", "red", "blue")) + theme(legend.title=element_blank(), legend.justification=c(1,1), legend.position=c(1,1), panel.background = element_rect(fill = NA), legend.background = element_rect(fill = NA), legend.key = element_rect(fill = "white"), legend.box.background = element_rect(colour = "black"), legend.key.width=unit(2,"line"))
  plot4 <- ggplot(powerdata, aes(Date, Global_reactive_power)) + geom_line() + scale_x_datetime(date_labels = "%a", date_breaks ="1 day") + xlab("datetime") + ylab("Global_reactive_power") + theme( panel.background = element_rect(fill = NA, colour = "black"))
